...
| `make mutants` | What the CI mutation workflows invoke (`GATE_JOBS` parallelism, `MUTANTS_BASE` diff base, `MUTANTS_SHARD` one 0-indexed shard such as `0/6`). A source-only list pass rejects stale line-pinned allowlist entries before any build. Each timeout gets one exact-mutant retry at twice its original test timeout; a one-line diff scopes structural mutants and a list precheck must name only the target before any build. Both runtimes are reported, and misses plus still-open retries reconcile against `scripts/mutants-allowlist.txt`. Refuses to start while another cargo-mutants runs anywhere on the machine. **Not for local use**: it saturates the machine for hours. |
...
- **Implementations and drafted specs/ADRs get sourced alix study decks**
  ...
  `<study-workspace>/decks/<issue-slug>.md` (the concrete workspace path
  lives in the private memory, not this tracked file; `decks/` is the
  workspace's fixed member directory); do not create a separate
...
- **Pre-1.0 has no backwards compatibility. This is a hard rule.** The installed
  ...
  deletes old artifacts. **Old formats are never recognized** (user ruling
  2026-08-02, tightening the earlier doctor-may-detect allowance): production
  and diagnostic code, `alix doctor` included, never matches on, names, or
  special-cases a retired key, grammar, or vocabulary; an old artifact fails as
  ordinary invalid input (unknown key, invalid id, unreadable document), with
  no dedicated message and no suggested remedy. No dual
