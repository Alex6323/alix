#!/usr/bin/env python3
"""stats.py: print alix's download numbers from GitHub releases and crates.io.

Read-only, no auth needed (both APIs are public), stdlib only (urllib.request,
json) so it runs anywhere `make stats` runs. Two independent sections; a
failure in one (rate limit, no network) prints a clear one-line message and
still lets the other section run.
"""
...
def fetch_json(url: str, user_agent: str) -> tuple[object | None, str | None]:
...
def main() -> int:
    ...
    print(f"Summary: {github_total} GitHub asset downloads + {crates_total} crates.io downloads "
          f"= {github_total + crates_total} total")
    print()
