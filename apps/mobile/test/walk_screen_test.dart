...
void main() {
  ...
  group('walking a trace end-to-end', () {
    ...
    testWidgets(
      'a checkpoint with no resolvable source shows the excerptError calmly',
      (tester) async {
  ...
  group('the exam handoff on the done screen', () {
        ...
        await tester.pumpWidget(
          MaterialApp(
            ...
            home: WalkScreen(
              ...
              supportDir: support,
              buildClient: (_) =>
                  FakeServerClient(versionReply: '0.6.0', examStartReply: true),
            ),
          ),
        );
        await tester.pumpAndSettle();
        ...
        await tester.pumpWidget(
          MaterialApp(
            ...
            home: WalkScreen(
              ...
              supportDir: support,
              buildClient: (_) =>
                  FakeServerClient(versionReply: null, examStartReply: true),
            ),
          ),
        );
        await tester.pumpAndSettle();
    ...
    testWidgets('an older paired server: "Take the exam" does not exist', (
      tester,
    ) async {
      final root = oneHopRoot();
      final support = tempSupport();
    ...
    testWidgets(
      ...
      'and Re-pair opens the pairing sheet, matching review_screen.dart',
      (tester) async {
        final root = oneHopRoot();
        final support = tempSupport();
        await setServer(
          const ServerConfig(host: '127.0.0.1', port: 7777, token: 'stale'),
          support: support,
        );
    ...
    testWidgets(
      'paired but cooling down from a recent failed sitting: a hint, not the button',
      (tester) async {
        final root = oneHopRoot();
        final support = tempSupport();
        await setServer(
  ...
  group('the picker routes a trace row to the walk', () {
    testWidgets(
      ...
      (tester) async {
        ...
        expect(
          find.text(
            'Trace decks are guided source walks; for now they '
            'live in the web app.',
          ),
          findsNothing,
        );
