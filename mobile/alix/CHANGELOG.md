...
- A drained Recognize sitting's summary note (cards waiting at Recall,
  cards needing answer choices) now reaches the app; the counts
  previously never crossed the bridge.

- Readable text on the Gruvbox Light and Catppuccin Latte themes: both
  inherited the dark palette's light-gray text and faint colors, which
  washed out against their light backgrounds; they now carry their own.

- The pairing-expired message pointed at "the deck list menu", which no
...
- Old deck formats are no longer specially recognized by the embedded core:
  a file with retired keys or un-prefixed ids reads as ordinary invalid or
  uninitialized input, with no dedicated messages. The bundled tutorial's
  wording is also synced with the desktop copy.
