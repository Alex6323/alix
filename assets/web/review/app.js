...
// The focused picker row's canonical name, shared with the sheet owner for
// actions that apply to a deck, workspace, or folder.
...
const idleInSelect = () =>
  state && state.phase === "select" && !browsing && !exam.isOpen() && !augment.isOpen() && !walk.isOpen() && !tutor.isOpen();
...
async function renderSelect(preloaded) {
  ...
  function renderList(opts) {
    ...
    function syncPrimary() {
      ...
      if (f && (f._item || f._open)) {
        ...
        ag.addEventListener("click", () => {
          ...
          augment.open(f._item ? f._item.name : f._open.name);
        });
    ...
    lists.addEventListener("keydown", (e) => {
      ...
      else if (e.key === "a" && cur && (cur._item || cur._open)) { e.preventDefault(); lastWorkspace = cur._wsName || null; augment.open(cur._item ? cur._item.name : cur._open.name); }
