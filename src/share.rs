...
pub(crate) const DECK_BUNDLE_MARKER: &str = ".alix-deck-share.json";
...
/// The exact number of source and generated bytes [`stage_path`] would place
/// in its projection, without copying those bytes or constructing a zip.
pub fn staged_size(path: &Path) -> Result<u64> {
    if path.is_dir() {
        return staged_dir_size(path);
    }
    if !path.is_file() {
        bail!("`{}` is neither a deck file nor a folder", path.display());
    }
...
/// The exact byte size of [`stage_deck_bundle`], including its generated
/// marker even when the initialized deck has no augmentation or owned assets.
pub fn staged_deck_bundle_size(path: &Path) -> Result<u64> {
    if !path.is_file() {
...
pub fn staged_deck_contents_size(path: &Path) -> Result<u64> {
    staged_deck_bundle_size(path)?
        ...
        .ok_or_else(|| anyhow::anyhow!("{} has an invalid bundle size", path.display()))
}

fn deck_bundle_marker(path: &Path) -> Result<String> {
    let file_name = path
        .file_name()
        .and_then(|name| name.to_str())
        .unwrap_or("deck.md");
    Ok(serde_json::to_string_pretty(&DeckBundle {
        version: DECK_BUNDLE_VERSION,
        deck: file_name.to_string(),
    })?)
}

fn deck_bundle_size(path: &Path, parts: &DeckBundleParts) -> Result<u64> {
    let marker = deck_bundle_marker(path)?;
    let mut bytes = std::fs::metadata(path)?.len() + marker.len() as u64;
    if parts.augmentation.is_file() {
        bytes += std::fs::metadata(&parts.augmentation)?.len();
    }
    if parts.has_assets {
        bytes += tree_size(&parts.owned_assets)?;
    }
    Ok(bytes)
}

fn tree_size(dir: &Path) -> Result<u64> {
    let mut bytes = 0;
...
pub fn zip_to(path: &Path, out: &Path) -> Result<usize> {
    ...
    if path.is_file() {
        ...
        copy_stream(&mut std::fs::File::open(path)?, &mut zip)?;
        entries = 1;
    } else {
        let root = path
            .file_name()
...
pub fn zip_contents_to(path: &Path, out: &Path) -> Result<usize> {
    ...
    let options: zip::write::SimpleFileOptions = Default::default();
    let mut entries = 0;
    zip_walk(&mut zip, path, "", options, &mut entries)?;
    zip.finish()?;
    Ok(entries)
}
...
fn zip_walk(
    ...
    for entry in children {
        ...
        if entry.path().is_dir() {
            ...
            zip.start_file(name, options)?;
            copy_stream(&mut std::fs::File::open(entry.path())?, &mut *zip)?;
            *entries += 1;
        }
    }
    Ok(())
...
mod tests {
    ...
    #[test]
    fn sync_zipping_writes_entry_relative_names_without_a_wrapper() {
        let dir = tempfile::tempdir().unwrap();
        let src = dir.path().join("german-verbs.md");
        std::fs::create_dir_all(src.join(".alix/progress")).unwrap();
        touch(&src, "german-verbs.md");
        touch(&src.join(".alix"), "pull.json");
        touch(&src.join(".alix/progress"), "deck-verbs.json");

        let out = dir.path().join("sync.zip");
        zip_contents_to(&src, &out).unwrap();
        ...
        assert_eq!(
            vec![
                ".alix/progress/deck-verbs.json",
                ".alix/pull.json",
                "german-verbs.md",
            ],
            names,
            "sync archive names are exactly entry-root-relative"
        );
    }

    #[test]
    fn zip_copy_buffer_is_constant_for_n_and_ten_n_bytes() {
        ...
        for bytes in [n, 10 * n] {
            ...
            assert_eq!(
                bytes as u64,
                copy_stream(&mut reader, std::io::sink()).unwrap(),
                "the streaming copy must account for every input byte at size {bytes}"
            );
            observed.push(reader.max_requested);
        }
        assert_eq!(
            vec![ZIP_COPY_BUFFER, ZIP_COPY_BUFFER],
            observed,
            "ten times the input must not increase the input buffer"
        );
    }
    ...
    fn staged_size_matches_the_bytes_stage_path_copies_without_copying_first() {
        ...
        let src = dir.path().join("ws");
        std::fs::create_dir_all(src.join("decks")).unwrap();
        std::fs::write(src.join("alix.toml"), "format-version = 1\n").unwrap();
        std::fs::write(
            src.join("decks/deck.md"),
            "---\nformat-version: 1\nid: deck-deck1\n---\n## q\na\n<!-- id: card-card1 -->\n",
        )
        ...
        let before = staged_size(&src).unwrap();
        let stage = dir.path().join("stage");
        let (staged, _) = stage_path(&src, &stage).unwrap();
        let after = byte_size(&staged);

        assert_eq!(
            before, after,
            "metadata projection and staged bytes diverged"
        );
        assert!(
    ...
    fn byte_size(path: &Path) -> u64 {
        ...
        std::fs::read_dir(path)
            .unwrap()
            .map(|entry| byte_size(&entry.unwrap().path()))
            .sum()
    }
