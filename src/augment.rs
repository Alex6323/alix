//! AI question augmentation: an opt-in layer that lets an LLM enrich a card's
//! *presentation* at review time without touching its identity or progress.
//!
//! The first and only augmentation today is **choice-mode distractors** —
//! plausible wrong answers generated by Claude, with the offline sampler in
//! [`crate::choice`] as the fallback. The layer is deliberately shaped to grow:
//! morphed question variants (a planned follow-up) will land as a second field
//! on [`Augmentation`], sharing this same cache and its id-keyed invalidation.
//!
