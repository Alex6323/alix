...
pub fn materialize(
    ...
    let mut manifest = String::from("# Generated by `alix workspace generate`.\n");
    ...
    for (item, name) in items.iter().zip(&names) {
            ...
            .filter(|s| !s.is_empty())
        {
            ...
            None => body.push_str(match item.kind {
                Kind::Trace => {
                    "Stub from `alix workspace generate`. Build the path:  alix deck generate <this file>\n"
                }
                Kind::Deck => {
                    "Stub from `alix workspace generate`. Author cards here, or `alix deck generate` from the source.\n"
                }
            }),
        }
...
";
    ...
    fn materialize_writes_filled_content_when_given() {
        ...
        let deck = fs::read_to_string(dir.join("decks/01-the-deck-format.md")).unwrap();
        assert!(deck.contains("Stub from `alix workspace generate`"));
