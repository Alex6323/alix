...
pub fn materialize(
    ...
    let mut manifest = String::from("# Generated by `alix generate workspace`.\n");
    ...
    for (item, name) in items.iter().zip(&names) {
            ...
            .filter(|s| !s.is_empty())
        {
            ...
            None => body.push_str(match item.kind {
                Kind::Trace => {
                    "Stub from `alix generate workspace`. Build the path:  alix generate deck <this file>\n"
                }
                Kind::Deck => {
                    "Stub from `alix generate workspace`. Author cards here, or `alix generate deck` from the source.\n"
                }
            }),
        }
...
";
    ...
    fn materialize_writes_filled_content_when_given() {
        ...
        let deck = fs::read_to_string(dir.join("decks/01-the-deck-format.md")).unwrap();
        assert!(deck.contains("Stub from `alix generate workspace`"));
