...
from orchestrator.validation import patch_paths
...
"""
    ...
    def test_build_output_in_the_stub_crate_never_reaches_the_property_patch(
        self,
    ) -> None:
        """Cargo writes `target/` and `Cargo.lock` the moment the property author
        runs its own suite. Those are not edits, and counting them rejects an
        author that obeyed the boundary."""
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            repo = target_repo(root)
            pin_formatting(repo)
            ...
            worktree = Path(state.agents["b"].worktree)
            baseline = git(worktree, "rev-parse", "HEAD")

            (worktree / "tests").mkdir(exist_ok=True)
            (worktree / "tests/properties.rs").write_text("// the author's work\n")
            (worktree / "target/debug").mkdir(parents=True)
            (worktree / "target/debug/build-artifact").write_text("cargo output\n")
            (worktree / "Cargo.lock").write_text("# generated by cargo\n")

            git(worktree, "add", "-N", ".")
            patch = git(worktree, "diff", "--binary", baseline)

            self.assertEqual(
                ["tests/properties.rs"],
                sorted(patch_paths(patch)),
                "cargo's own output was counted as an edit by the property author",
            )

    def test_harness_formatting_of_the_stub_api_is_not_blamed_on_the_author(
        self,
    ) -> None:
        """`cargo fmt --all` reflows src/lib.rs, which the harness wrote verbatim
        from the spec. Left in the tree it becomes a non-tests/ path in the next
        round's patch, rejecting an author that never touched it."""
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            repo = target_repo(root)
            pin_formatting(repo)
            ...
            worktree = Path(state.agents["b"].worktree)
            (worktree / "src/lib.rs").write_text("pub fn add() -> i32 {\n    0\n}\n")
            (worktree / "tests").mkdir(exist_ok=True)
            (worktree / "tests/properties.rs").write_text("// the author's work\n")
            ...
            self.assertEqual(
                ["tests/properties.rs"],
                sorted(patch_paths(patch)),
                "the harness's own reformatting stayed in the author's diff",
            )
            self.assertIn(
                "pub fn add() -> i32 {",
                git(worktree, "show", "HEAD:src/lib.rs"),
                "the formatted API must be preserved, not reverted",
            )

    def test_the_implementer_is_told_where_the_property_suite_imports_the_api(
        self,
    ) -> None:
        """The property author is told the stub is the complete contract, and the
        stub exposes the API at the crate root. Unless the implementer is told to
        keep it reachable there, a suite written against the stub cannot compile
        against the implementation."""
        implement = (_prompt_dir() / "implement.md").read_text(encoding="utf-8")
        self.assertIn("crate root", implement)
