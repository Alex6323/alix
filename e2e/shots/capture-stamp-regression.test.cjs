...
test("fabricated graduation includes a valid span whose hidden value contains greater-than", () => {
  ...
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "alix-capture-repro-"));
  ...
  assert.ok(
    ...
    "the valid current-format blank sub-card must be graduated",
  );
});
