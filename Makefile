...
# Triage mail forwarded from contact@alix.study (read-only IMAP; credentials
# in ~/.config/alix-mail/credentials, never in the repo). Maintainer-only.
check-mail:
...
parser-hostility-test:
	cargo test --lib parser::tests::one_authored_base_id_remains_legal_for_every_derived_or_inert_shape
