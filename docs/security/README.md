...
missing lockfile integrity data. It also reads source-affecting tables from either root Cargo
configuration spelling, tracked or untracked. `make deps-check` retains the reviewed Cargo
version-family baseline.
...
`src/sync/server.rs`). A link directly under `assets/`, an `assets/` root
that is itself a link, and an unreadable `assets/` root are refused by
staging, and doctor reports each as an error rather than as left out
(`a_linked_orphan_asset_directory_is_reported_as_a_link`,
`a_linked_assets_root_is_reported_as_a_link`,
`an_unreadable_assets_root_is_a_workspace_error`, `src/cli/doctor.rs`).
...
| A received ZIP attempts path traversal | The `zip` crate's extraction rejects unsafe enclosed paths, which bounds where an entry is written but not where a link entry points, so alix itself refuses a received archive carrying a symbolic link before the sanitizer walks and before anything is moved; receive then strips personal-state files. | Treat the archive and external transfer tool as untrusted; inspect received content before opening or enabling AI. |
