...
| A fact whose common confusions are known and nameable. | Authored multiple choice: a task list with one `- [x]` and one or more `- [ ]`, ideally two or three. | judgement | The distractors are the teaching. Write them only if you can say what mistaken belief each one represents; if you cannot, the shape is doing nothing. |
