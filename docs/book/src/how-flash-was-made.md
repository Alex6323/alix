...
flash was built entirely through conversation. There was no team and no
...
flash is a ground-up rewrite of an earlier flashcard tool — one the author wrote
...
We say so plainly because it's the same bet the tool itself makes. flash holds
...
test suite and the lints are the floor, and the author uses flash to learn real
