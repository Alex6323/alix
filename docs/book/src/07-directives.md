...
| `plain` | card · section | On the line below a task list, a table, or a thematic break of any spelling: keeps that one shape literal, escaping a deck default (or, for a break, the divider and thematic-break grammar in every position, including where the break would otherwise be an error). |
