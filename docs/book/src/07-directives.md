...
| `choices` | deck | Deck-wide default task-list mapping: `single` or `multiple`. A per-card invocation overrides it. [→ ch 3](03-the-deck-format.md#choice-cards-task-lists) |
...
| `id` | card | The HTML-comment card ID (`card-<token>`) anchors review history. It is minted by `alix deck init` or a deck-creation workflow and maintained by alix, never hand-authored. After a card table it is the table's container ID; each row's card composes it with the row stamp (`r:`) at the end of the row, after the closing pipe. [→ ch 3](03-the-deck-format.md) |
